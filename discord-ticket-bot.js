...
const CONFIG = {
    ...
    STAFF_ROLE_ID: process.env.STAFF_ROLE_ID,          // Роль для модераторов
    LOG_CHANNEL_ID: process.env.LOG_CHANNEL_ID,         // Канал для логов действий
    CREATOR_ROLE_ID: process.env.CREATOR_ROLE_ID,        // Роль для выдачи монет/опыта
    
    // 🎫 СИСТЕМА ТИКЕТОВ
    TICKET_CATEGORY_ID: process.env.TICKET_CATEGORY_ID,     // Категория для тикетов
    TICKETS_CHANNEL_ID: process.env.TICKETS_CHANNEL_ID,     // Канал для создания тикетов
    
    // 🎙️ ГОЛОСОВЫЕ КАНАЛЫ
    VOICE_CATEGORY_ID: process.env.VOICE_CATEGORY_ID,      // Категория для временных голосовых комнат (замените на вашу)

    
    // 👋 ПРИВЕТСТВИЕ
    WELCOME_CHANNEL_ID: process.env.WELCOME_CHANNEL_ID,     // Канал приветствия новых участников
    
    // 📺 YOUTUBE
    YOUTUBE_NOTIFICATION_CHANNEL_ID: process.env.YOUTUBE_NOTIFICATION_CHANNEL_ID, // Канал уведомлений о новых видео
    ...
    // 👑 ПРЕМИУМ РОЛИ
    VIP_ROLE_ID: process.env.VIP_ROLE_ID,        // VIP статус (x2 бонус монет)
    PREMIUM_ROLE_ID: process.env.PREMIUM_ROLE_ID,    // Premium статус (50 монет / 12ч)
    OLDTIMER_ROLE_ID: process.env.OLDTIMER_ROLE_ID    // Старичок (150 монет / 24ч)
};
...
(async () => {
    try {
        ...
        if (guildId) {
            // Быстрая синхронизация для конкретного сервера (несколько секунд)
            await rest.put(
            ...
            // Глобальная синхронизация (может занять до 1 часа)
            await rest.put(
...
let userData = {};
let youtubeData = { lastVideoId: null, lastCheck: Date.now() };
...
function loadData() {
    try {
        ...
        if (fs.existsSync(YOUTUBE_FILE)) {
            youtubeData = JSON.parse(fs.readFileSync(YOUTUBE_FILE, 'utf8'));
        }
    } catch (error) {
...
function saveData() {
    try {
        ...
        fs.writeFileSync(YOUTUBE_FILE, JSON.stringify(youtubeData, null, 2));
    } catch (error) {
...
// ==================== АВТО-РОЛЬ (сохранение в файл) ====================
const AUTO_ROLE_FILE = path.join(DATA_DIR, 'auto_role.json');

function loadAutoRole() {
    try {
        if (fs.existsSync(AUTO_ROLE_FILE)) {
            const d = JSON.parse(fs.readFileSync(AUTO_ROLE_FILE, 'utf8')) || {};
            if (d.autoRoleId) CONFIG.AUTO_ROLE_ID = d.autoRoleId;
        }
    } catch (e) { console.error('Ошибка загрузки авто-роли:', e); }
}

function saveAutoRole() {
    try {
        fs.writeFileSync(AUTO_ROLE_FILE, JSON.stringify({ autoRoleId: CONFIG.AUTO_ROLE_ID || '' }, null, 2));
    } catch (e) { console.error('Ошибка сохранения авто-роли:', e); }
}
...
async function checkNewVideo() {
    try {
        ...
        if (data.items && data.items.length > 0) {
            ...
            if (videoId && videoId !== youtubeData.lastVideoId) {
                youtubeData.lastVideoId = videoId;
                youtubeData.lastCheck = Date.now();
                saveData();
...
client.on('messageCreate', async (message) => {
    ...
    if (!cooldown || now - cooldown >= LEVEL_CONFIG.XP_COOLDOWN) {
        ...
        if (result.leveledUp) {
            ...
            for (const [level, roleId] of Object.entries(CONFIG.LEVEL_ROLES)) {
                if (result.newLevel >= parseInt(level) && !message.member.roles.cache.has(roleId)) {
                    try {
    ...
    // ==================== КОМАНДЫ !SETUP (ТОЛЬКО ДЛЯ АДМИНОВ) ====================
        ...
        try {
            ...
            if (!statusCategory) {
                statusCategory = await guild.channels.create({
                    ...
                    position: 0  // Самая верхняя позиция
                });
                console.log('✅ Категория Статус создана в самом верху');
            } else {
                // Если категория существует, переместим её в верх
                await statusCategory.setPosition(0).catch(() => {});
            }

            // Получаем текущие статистические данные
            const totalMembers = guild.memberCount;
            const voiceMembers = guild.members.cache.filter(m => m.voice.channel).size;  // Подсчитываем только тех, кто реально в голосе
            const boosts = guild.premiumSubscriptionCount || 0;

            // Создаём или обновляем каналы статуса
            ...
            for (const config of channelConfigs) {
                // Ищем канал по точному совпадению имени паттерна
                ...
                // Закрываем канал для всех (никто не может подключиться)
...
// temporary voice rooms tracking (in-memory)
const tempVoiceRooms = {};

client.on('voiceStateUpdate', async (oldState, newState) => {
    const userId = newState.id;
    
    if (!oldState.channel && newState.channel) {
        // Создание временной голосовой комнаты при подключении к специальному каналу
        try {
    ...
    if (oldState.channel && !newState.channel) {
        // Удаляем временную комнату если она пуста
        try {
            const oldCh = oldState.channel;
            if (oldCh && tempVoiceRooms[oldCh.id]) {
                if (oldCh.members.size === 0) {
                    delete tempVoiceRooms[oldCh.id];
    ...
    // Обновляем статус-каналы при изменении голоса
...
client.on('guildMemberAdd', async (member) => {
    ...
    // Авто-роль: если указана, выдаём новую роль при входе
    try {
...
// (Обработка перемещена в основной обработчик ниже)

// УДАЛЕНО - смотрите второй обработчик interactionCreate для всех обработок

// ==================== ОСНОВНОЙ ОБРАБОТЧИК SLASH КОМАНД И ВЗАИМОДЕЙСТВИЙ ====================
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        const helpEmbed = new EmbedBuilder()
            ...
            .setDescription(
                ...
                `**💰 Экономика:**\n` +
                `/balance — показать баланс (монеты и банк)\n` +
                `/transfer <user> <amount> — передать монеты другому пользователю\n` +
                `/bank deposit <amount> — положить монеты в банк\n` +
                `/bank withdraw <amount> — снять монеты из банка\n` +
                `Админ: /bank apply_interest <percent> — начислить процент на все банковские счета\n\n` +
                `**🛒 Магазин и премиум:**\n` +
                `/shop — открыть магазин\n` +
                `/inventory — показать покупки\n` +
                `/premium — получить премиум-нагрузку (50 монет / 12ч, для держателей роли)\n` +
                `/oldtimer — получить награду Старичка (150 монет / 24ч)\n\n` +
                `**🛡️ Модерация:**\n` +
                `/warn <user> [reason] — выдать предупреждение (логируется)\n` +
                `/mute <user> [minutes] — замутить пользователя\n` +
                `/unmute <user> — снять мут\n` +
                `/kick <user> [reason] — кикнуть из сервера\n` +
                `/ban <user> [reason] — забанить пользователя\n\n` +
                `**🎙️ Голосовые комнаты:**\n` +
                `На сервере работает канал "Присоединиться для создания" — подключитесь к нему, и бот автоматически создаст новую временную комнату **Room - ВашеИмя** и переместит вас туда.\n\n`
                `📺 **YouTube:** [ИГРАЕМ ВМЕСТЕ](${CONFIG.YOUTUBE_CHANNEL})`
            )
    ...
    if (commandName === 'adminhelp') {
        ...
        const adminEmbed = new EmbedBuilder()
            ...
            .setDescription(
                ...
                `/warn <user> [reason] — выдать предупреждение (логируется в канал логов)\n` +
                `/mute <user> [minutes] — замутить пользователя (роль Muted создается автоматически)\n` +
                `/unmute <user> — снять мут\n` +
                `/kick <user> [reason] — кикнуть пользователя\n` +
                `/ban <user> [reason] — забанить пользователя\n\n` +
                `**💰 Деньги и опыт (роль: Creator):**\n` +
                `/give <user> <amount> — выдать монеты пользователю\n` +
                `/addex <user> <amount> — добавить опыт (XP) пользователю\n` +
                `/addlevel <user> <levels> — добавить уровни пользователю\n\n` +
                `**🏦 Банк:**\n` +
                `/bank apply_interest <percent> — начислить процент на все банковские счета\n\n` +
                `**🔧 Системные функции:**\n` +
                `• Автоматическая выдача ролей за уровни\n` +
                `• Система XP за сообщения и голос\n` +
                `• Уведомления о новых YouTube видео\n` +
                `• Магазин с ролями за монеты`
            )
    ...
    if (commandName === 'rank') {
        ...
        const currentLevelXP = user.xp - (user.xp >= requiredXP ? Math.floor(user.xp / requiredXP) * requiredXP : 0);
    ...
    if (commandName === 'transfer') {
        ...
        if (!target || !amount || amount <= 0) return interaction.reply({ content: '❌ Неверные параметры.', ephemeral: true });
        const from = getUserData(interaction.user.id);
        const to = getUserData(target.id);
        if (from.coins < amount) return interaction.reply({ content: '❌ Недостаточно монет.', ephemeral: true });
        from.coins -= amount;
        to.coins += amount;
        saveData();
        await interaction.reply({ content: `✅ ${interaction.user.tag} передал ${amount} монет ${target.tag}.` });
        try { await logModAction(interaction.guild, `💸 ${interaction.user.tag} передал ${amount} монет ${target.tag}`); } catch(e){}
    }
    ...
    if (commandName === 'bank') {
        ...
        if (sub === 'deposit') {
            const amount = interaction.options.getInteger('amount');
            if (!amount || amount <= 0 || user.coins < amount) return interaction.reply({ content: '❌ Неверная сумма или недостаточно монет.', ephemeral: true });
            user.coins -= amount; user.bank += amount; saveData();
            return interaction.reply({ content: `✅ Вы положили ${amount} в банк. Баланс в банке: ${user.bank} монет.` });
        }
        if (sub === 'withdraw') {
            const amount = interaction.options.getInteger('amount');
            if (!amount || amount <= 0 || user.bank < amount) return interaction.reply({ content: '❌ Неверная сумма или недостаточно средств в банке.', ephemeral: true });
            user.bank -= amount; user.coins += amount; saveData();
            return interaction.reply({ content: `✅ Вы сняли ${amount} из банка. Остаток в банке: ${user.bank} монет.` });
        }
        if (sub === 'apply_interest') {
            if (!interaction.member.permissions.has(PermissionFlagsBits.Administrator)) return interaction.reply({ content: '❌ Только админы.', ephemeral: true });
            const percent = interaction.options.getInteger('percent');
            if (!percent || percent <= 0) return interaction.reply({ content: '❌ Неверный процент.', ephemeral: true });
    ...
    // ==================== МОДЕРАЦИЯ: WARN / MUTE / UNMUTE / KICK / BAN ====================
    if (commandName === 'warn') {
        if (!interaction.member.roles.cache.has(CONFIG.STAFF_ROLE_ID) && !interaction.member.permissions.has(PermissionFlagsBits.Administrator)) return interaction.reply({ content: '❌ Нет прав.', ephemeral: true });
        const target = interaction.options.getUser('user');
    ...
    if (commandName === 'give') {
        if (!interaction.member.roles.cache.has(CONFIG.CREATOR_ROLE_ID) && !interaction.member.permissions.has(PermissionFlagsBits.Administrator)) return interaction.reply({ content: '❌ Нет прав.', ephemeral: true });
        const target = interaction.options.getUser('user');
        const amount = interaction.options.getInteger('amount');
        if (!target || !amount || amount <= 0) return interaction.reply({ content: '❌ Неверные параметры.', ephemeral: true });
    ...
    if (commandName === 'addlevel') {
        ...
        if (!target || !levels || levels <= 0) return interaction.reply({ content: '❌ Неверные параметры.', ephemeral: true });
        const targetUser = getUserData(target.id);
        const oldLevel = targetUser.level;
        targetUser.level += levels;
        targetUser.xp = getRequiredXP(targetUser.level);
        saveData();
        ...
        for (let i = 0; i < Math.min(SHOP_ITEMS.length, 5); i++) {
            buttons.push(
                new ButtonBuilder()
                    ...
                    .setLabel(`${SHOP_ITEMS[i].emoji} ${SHOP_ITEMS[i].name}`)
                    .setStyle(ButtonStyle.Primary)
            );
        }
    ...
    // ==================== /ROOM (temporary voice room management) ====================
    if (commandName === 'room') {
        ...
        if (!tempVoiceRooms[vc.id]) return interaction.reply({ content: '❌ Эта команда работает только для временных комнат, созданных ботом.', ephemeral: true });

        const ownerId = tempVoiceRooms[vc.id].owner;
        ...
        try {
            ...
            if (sub === 'claim') {
                tempVoiceRooms[vc.id].owner = interaction.user.id;
    ...
    }  // закрытие if (interaction.isChatInputCommand())
    
    // ==================== ОБРАБОТКА КНОПОК И МОДАЛОВ ====================
    // ПОКУПКИ В МАГАЗИНЕ
    if (interaction.isButton() && interaction.customId.startsWith('shop_buy_')) {
        ...
        if (user.coins < item.price) {
            return interaction.reply({ 
                content: `❌ Недостаточно монет!\n\n💰 У вас: **${user.coins}**\n💎 Нужно: **${item.price}**\n\n💡 Зарабатывайте монеты активностью на сервере!`, 
                ephemeral: true 
            });
        }
        ...
        if (item.type === 'role' && item.roleId) {
            try {
                ...
                console.error('Ошибка выдачи роли:', error);
                return interaction.reply({ 
                    content: '❌ Ошибка при выдаче роли! Обратитесь к администрации.\n💰 Монеты были возвращены.', 
                    ephemeral: true 
                });
            }
        }
    ...
    // Visit instruction button
    if (interaction.isButton() && interaction.customId === 'join_to_create_panel') {
    ...
    // Voice control buttons (admin/owner)
    if (interaction.isButton() && ['vc_lock','vc_unlock','vc_rename','vc_limit','vc_claim'].includes(interaction.customId)) {
        ...
        if (!tempVoiceRooms[memberVc.id]) return interaction.reply({ content: '❌ Целевая комната не является временной, созданной ботом.', ephemeral: true });
        ...
        try {
            ...
            if (id === 'vc_claim') {
                const oldOwner = tempVoiceRooms[memberVc.id].owner;
                tempVoiceRooms[memberVc.id].owner = interaction.user.id;
                const claimEmbed = new EmbedBuilder()
                    .setColor(COLORS.PRIMARY)
                    .setTitle('🤝 Владение трансферировано')
                    .setDescription(`Вы теперь администратор комнаты **${memberVc.name}**.\n\n📋 Теперь вы можете ополнять права владения.`)
                    .setFooter({ text: 'Управление комнатами' })
                    .setTimestamp();
                await interaction.reply({ embeds: [claimEmbed], ephemeral: true });
                await logModAction(interaction.guild, `🤝 ${interaction.user.tag} СТАЛ НОВЫМ ВНОВОМ КОмнаты **${memberVc.name}** (Пропю владенец: <@${oldOwner}>)`);
                return;
            }
    ...
    // ТИКЕТЫ (весь оригинальный код остается без изменений)
    if (interaction.isButton() && interaction.customId === 'start_ticket') {
        const selectMenu = new StringSelectMenuBuilder()
            .setCustomId('select_category')
            .setPlaceholder('Выберите категорию обращения')
            .addOptions(TICKET_CATEGORIES.map(cat => ({ label: cat.label, value: cat.value, emoji: cat.emoji })));
    ...
    if (interaction.isModalSubmit()) {
        ...
        if (interaction.customId === 'modal_video_idea') {
        ...
        else if (interaction.customId === 'vc_rename_modal') {
            const newName = interaction.fields.getTextInputValue('vc_new_name');
            const vc = interaction.member?.voice?.channel || (interaction.guild.members.cache.get(interaction.user.id)?.voice?.channel);
            if (!vc) {
                await interaction.editReply({ content: '❌ Вы не находитесь в голосовом канале для выполнения этой операции.', ephemeral: true }).catch(()=>{});
                return;
            }
            if (!tempVoiceRooms[vc.id]) {
                await interaction.editReply({ content: '❌ Эта команда работает только для временных комнат, созданных ботом через панель управления.', ephemeral: true }).catch(()=>{});
                return;
            }
            ...
            const renameEmbed = new EmbedBuilder()
                ...
                .setDescription(`**Старое имя:** Комната\n**Новое имя:** ${newName}\n\n✅ Все участники видят новое имя в реальном времени.`)
                .setFooter({ text: 'Управление комнатами' })
                .setTimestamp();
            await interaction.editReply({ embeds: [renameEmbed], ephemeral: true }).catch(()=>{});
            ...
            if (size < 0 || size > 99) {
                await interaction.editReply({ content: '❌ Лимит должен быть от 0 до 99 человек.', ephemeral: true }).catch(()=>{});
                return;
            }
            ...
            const limitEmbed = new EmbedBuilder()
                ...
                .setFooter({ text: 'Управление комнатами' })
                .setTimestamp();
            await interaction.editReply({ embeds: [limitEmbed], ephemeral: true }).catch(()=>{});
...
client.on('ready', () => {
    ...
    // Запускаем обновление статус-каналов каждые 5 минут
